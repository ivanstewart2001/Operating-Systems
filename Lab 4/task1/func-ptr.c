...
int my_comparer(const void *this, const void *that){
  ...
  else if ((secondProcess.priority) > (firstProcess.priority)){
    return 1;
  } else {
    if ((firstProcess.arrival_time) < (secondProcess.arrival_time)) {
      return -1;
      } 
    
    else if ((secondProcess.arrival_time) < (firstProcess.arrival_time)){
      return 1;
    }
